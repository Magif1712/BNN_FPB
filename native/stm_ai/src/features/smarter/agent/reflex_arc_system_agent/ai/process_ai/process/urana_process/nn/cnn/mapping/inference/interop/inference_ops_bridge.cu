    ...
    intptr_t traceZ, intptr_t traceY)
{
    ...
    if (traceZ == 0) {
        // NoTrace: z 用 traceY 做工作区（push→pull→activate 覆盖 traceY）
        cnn_forward_layer_notrace(
            ...
            sizeA0, sizeA1, (cudaStream_t)stream /* -> */, trace_y_vec->data()
        );
    } else {
        // StoreTrace: z=traceZ 累加，traceY 写 σ(z)。
        // traceY 即调用方的 ys[i]（kernel 直接写输出，零中转）。
        Vector<float> *trace_z_vec = reinterpret_cast<Vector<float>*>(traceZ);
        cnn_forward_layer_trace(
            x_vec->data(), q_vec->data(), l_vec->data(), r_vec->data(), b_vec->data(),
            idx0_vec->data(), idx1_vec->data(), w0_vec->data(), w1_vec->data(),
            sizeA0, sizeA1, (cudaStream_t)stream /* -> */, trace_z_vec->data(), trace_y_vec->data()
        );
    }
