...
// trace_y 写 σ(z)。trace_y 即调用方的 ys[i]（一份写入，extractC/backward 两方消费）。
// 前向用 idx/w（持久缓存），不用 p（p 仅 refreshCache/backward 用）。
// 重构：删去冗余 y 参数——StoreTrace 下 trace_y 就是输出，无需额外 y 中转站。
