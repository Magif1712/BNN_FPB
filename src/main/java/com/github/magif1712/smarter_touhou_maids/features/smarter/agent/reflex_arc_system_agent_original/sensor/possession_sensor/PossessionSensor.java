package com.github.magif1712.smarter_touhou_maids.features.smarter.agent.reflex_arc_system_agent_original.sensor.possession_sensor;
...
import com.github.magif1712.smarter_touhou_maids.features.smarter.agent.reflex_arc_system_agent_original.sensor.possession_sensor.vision.VisionOps;
 ...
 * <ul>
 *   <li><b>vision（去看）</b>：{@link VisionOps#captureScreenViaSnapshot} 一步完成
 *       快照（glBlit 深拷贝）+ 位平面编码，即"视网膜采集与编码"。这是感受器的视觉子模式，
 ...
 * <p>
 * <b>本类职责</b>：仅编排 vision 采集，把共享资源（feelingBuffer/cudaStream/completionEvent）
 * 与捕获专用资源（快照纹理）接起来。possession 子模式的状态管理由外周
 * （PossessionManager / SmarterClientService 事件入口）负责，本类不感知——
 * 这是"possession 是感受器的模式"而非"感受器是 possession 的模式"的层级关系：
 * possession 在 possession_sensor 内部。
 * <p>
 * <b>资源归属</b>：feelingBuffer/cudaStream/completionEvent 由 agent 注入
 * （共享，agent 是 owner）；快照纹理由本类在 {@link #awaken} 自建、{@link #shutdown} 自释
 * （捕获专用，隔离 OpenGL 源纹理竞态）。
 */
public class PossessionSensor implements ISensor {
    ...
    public void awaken(VectorBase feelingBuffer, Stream cudaStream, Event completionEvent) {
        // 本感受器编码固定位平面（BoolVector）——awaken 契约为 VectorBase，
        // 编码是感受器的私有模式，故在此向下转型（类型错误时 fail-fast）。
        this.feelingBuffer = (BoolVector) feelingBuffer;
    ...
    public void capture(int textureId, int texWidth, int texHeight) {
        VisionOps.captureScreenViaSnapshot(
                textureId, snapshotTexture, feelingBuffer,
                new Span(0, feelingBuffer.size()) {},
                texWidth, texHeight, cudaStream);
