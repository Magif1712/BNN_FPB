...
public abstract class AbstractBnnNeuralNetwork implements INeuralNetwork {
    ...
    private static final int INHERITANCE_MULTIPLIER = 3;             // C = F × 3（BNN 编码知识，藏于家族内部）
    ...
    public VectorBase newBehaviorBuffer(int behaviorLength) {
        return BoolVector.mapped(behaviorLength);
    }

    /**
     * BNN 行为读取：从 mapped BoolVector 零拷贝读 bit-packed int[]（纯 host memcpy，零 CUDA 调用）。
     * 与 CNN 的 readMappedToJava 对称——mapped memory 保证渲染线程不因 cudaMemcpy 隐式同步而阻塞。
    ...
    public void readBehaviorTo(VectorBase behaviorBuffer, int[] dst, long stream) {
        ...
        bv.readMappedToJava(dst, wordCount);
    }
    ...
    public void backward(Object fwTraceForBw, VectorBase y, VectorBase t, long stream /* -> */, VectorBase bufTc, Object bufHp) {
        ...
        // 阶段一：把输入层梯度的 C 区间（IntVector ±1）negateAndBinarize 为前向 C（BoolVector 0/1）外拷到 bufTc
        // nn 自己知道 C 在输入层梯度中的区间（offset=0, length=cLen），与 CNN native 内部用 sizeC 同构。
        if (bufTc != null) {
            int cLen = BNN_FEELING_LENGTH * INHERITANCE_MULTIPLIER;
            if (bufTc.size() != cLen) {
                throw new IllegalArgumentException(
                        "bufTc.size()=" + bufTc.size() + " != cLen=" + cLen
                        + " (BNN_FEELING_LENGTH=" + BNN_FEELING_LENGTH
                        + " * INHERITANCE_MULTIPLIER=" + INHERITANCE_MULTIPLIER + ")");
            }
            Span cSpanInInputGrad = new Span(0, cLen) {};
            this.gradientToInputFromInternal(cSpanInInputGrad, stream /* -> */, bufTc);
        }
    }
