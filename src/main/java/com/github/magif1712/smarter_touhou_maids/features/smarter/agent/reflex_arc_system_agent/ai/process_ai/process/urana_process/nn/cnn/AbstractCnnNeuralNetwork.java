package com.github.magif1712.smarter_touhou_maids.features.smarter.agent.reflex_arc_system_agent.ai.process_ai.process.urana_process.nn.cnn;
...
import com.github.magif1712.smarter_touhou_maids.features.smarter.agent.reflex_arc_system_agent.ai.process_ai.process.urana_process.nn.cnn.containers.io.CnnIO;
import com.github.magif1712.smarter_touhou_maids.features.smarter.agent.reflex_arc_system_agent.ai.process_ai.process.urana_process.nn.cnn.containers.io.CnnIOLayerGradients;
import com.github.magif1712.smarter_touhou_maids.features.smarter.agent.reflex_arc_system_agent.ai.process_ai.process.urana_process.nn.cnn.containers.io.value.CnnTargetVector;
import com.github.magif1712.smarter_touhou_maids.features.smarter.agent.reflex_arc_system_agent.ai.process_ai.process.urana_process.nn.cnn.mapping.inference.CnnInferenceOps;
import com.github.magif1712.smarter_touhou_maids.features.smarter.agent.reflex_arc_system_agent.ai.process_ai.process.urana_process.nn.cnn.mapping.training.CnnTrainingOps;
...
public abstract class AbstractCnnNeuralNetwork implements INeuralNetwork {
    ...
    public AbstractCnnNeuralNetwork(int inputSize, int outputSize, CnnNetworkData networkData) {
        ...
        int sizeA0 = this.networkData.getHyperparameters().getSizeA0();
        int sizeA1 = this.networkData.getHyperparameters().getSizeA1();
        this.io = new CnnIO(sizeA0, sizeA1);
        this.gradients = new CnnIOLayerGradients(sizeA0, sizeA1);
        this.target = new CnnTargetVector(sizeA1);
    ...
    public void forward(VectorBase x, long stream /* -> */, VectorBase y, Object fwTraceForBw) {
        ...
        // DPS 契约：kernel 始终写 y（forward 后 y 即为结果，上层无需 copyFromOutput 中转）
        // fwTraceForBw 非 null 时 StoreTrace（同时存 z），null 时 NoTrace（纯推理）
        CnnInferenceOps.cnnForwardLayer(io.getInput().getVector(), hp, stream /* -> */, (FloatVector) y, (CnnFwTraceForBw) fwTraceForBw);
    }
    ...
    @Override
    public void gradientToInputFromInternal(Span region, long stream /* -> */, VectorBase inputC) {
        // TODO 待 C 侧/设计落地
    }
    ...
    public CnnFwTraceForBw createFwTraceForBw() {
        // z 同 sizeA1：前向写 z（pre-activation），反向暂不读（sigmoid 用 y(1-y)，
        // 换 clipped linear 时需读 z 判断 |z|<1）。
        // 重构：y 不再分配——StoreTrace 下 trace_y 即 ys[i]（调用方注入）。
        int sizeA1 = this.networkData.getHyperparameters().getSizeA1();
        return new CnnFwTraceForBw(new FloatVector(sizeA1));
    }
